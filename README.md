...
| /gift t | gift.admin |   设置发放时间   |
| /gift st | gift.admin |   显示个人统计   |
| /gift top | gift.admin |   显示排行榜   |
| /gift cond | gift.admin |   显示进度参考   |
| /gift rs | gift.admin |   重置在线礼包   |
...
加入了排行榜与统计功能，支持持久化保存。
在线礼包支持进度限制功能。
修复了菜单指令显示问题，移除s查询剩余时间指令，直接集成在菜单指令下方
修复了生命上限检测逻辑错误导致无法发出礼包的问题
注意：配置礼包表里的物品名称不需要写，reload后会自动写入
